package com.nhsoft.neptune.api.domain;

import com.nhsoft.neptune.ApiCloudObject;
import com.nhsoft.neptune.ApiCloudField;

import java.math.BigDecimal;

import java.util.*;

import lombok.Data;

import lombok.EqualsAndHashCode;

/**
 * FileSaveDTO模型
 */
@Data
@EqualsAndHashCode(callSuper = true)
public class FileSaveDTOModel extends ApiCloudObject {

    @ApiCloudField(value="门店编号", example="99", required=true)
    private Integer branchNum;
    @ApiCloudField(value="业务类型(会员卡结算|供应商协议|供应商商品调价|供应商商品调价单|供应商管理|内部申请单|加盟店费用|售后申请|商品档案|商品调价|客户申请|客户费用|客户预收|库存盘点单|批发退货|批发销售单|收货单|新品申请|盘点单|调入单|调出单|调整单|退货单|邮件通知|采购订单|门店客户费用|门店补货|预盘单|批发订单)", example="采购订单", required=true)
    private String bizType;
    @ApiCloudField(value="业务单据号", example="CP123123123131", required=true)
    private String bizFid;
    @ApiCloudField(value="备注", example="备注")
    private String itemMemo;
    @ApiCloudField(value="关联主体(如采购订单的主体是供应商名称)", example="供应商名称")
    private String bizSubject;
    @ApiCloudField(value="操作人编号", required=true)
    private Integer appUserNum;
    @ApiCloudField(value="文件信息(base64编码)", required=true)
    private String fileContent;
    @ApiCloudField(value="文件名称，含后缀", example="xxx.jpg", required=true)
    private String fileName;
    @ApiCloudField(value="商品编号")
    private Integer itemNum;
    @ApiCloudField(value="商品批次")
    private String itemLotNumber;
}
