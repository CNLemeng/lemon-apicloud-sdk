...
/**
 * ClientSettlementFindV2DTO模型
 * 客户结算查询参数
 */
@Data
@EqualsAndHashCode(callSuper = true)
public class ClientSettlementFindV2DTOModel extends ApiCloudObject {

    @ApiCloudField(value="门店编码（时间类型为最后修改时间允许为空）", example="99")
    private Integer branchNum;
    @ApiCloudField(value="时间类型(制单时间|审核时间|付款日期|收款日期|最后修改时间)", example="审核时间", required=true)
