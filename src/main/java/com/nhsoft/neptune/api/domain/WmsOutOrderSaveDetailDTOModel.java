...
/**
 * WmsOutOrderSaveDetailDTO模型
 * WMS出库单保存明细
 */
@Data
@EqualsAndHashCode(callSuper = true)
public class WmsOutOrderSaveDetailDTOModel extends ApiCloudObject {

    @ApiCloudField(value="库位编码", example="8070", required=true)
    private Long positionNum;
    @ApiCloudField(value="库位名称", example="jh001", required=true)
    ...
    @ApiCloudField(value="出库数量（小数点3位）", example="1.0", required=true)
    private BigDecimal itemUseQty;
    @ApiCloudField(value="出库单位", example="公斤", required=true)
    private String itemUseUnit;
    @ApiCloudField(value="出库单价（小数点4位）", example="1.0", required=true)
